...
func (p *Proxy) replyChannelOpen(ctx context.Context, f *amqp091.MethodFrame, _ *amqp091.ChannelOpen) error {
	id := f.Channel()
	p.logger.Debug("Channel.Open", "channel", id, "proxy", p.id)
...
func (p *Proxy) replyChannelClose(_ context.Context, f *amqp091.MethodFrame, _ *amqp091.ChannelClose) error {
	id := f.Channel()
	p.logger.Debug("Channel.Close", "channel", id)
...
func (p *Proxy) replyQueueDeclare(_ context.Context, f *amqp091.MethodFrame, m *amqp091.QueueDeclare) error {
	...
	p.logger.Debug("Queue.Declare", "queue", m.Queue, "messages", messages, "consumers", consumers)
...
func (p *Proxy) replyBasicConsume(ctx context.Context, f *amqp091.MethodFrame, m *amqp091.BasicConsume) error {
	...
	p.logger.Debug("Basic.Consume", "queue", m.Queue)
...
func (p *Proxy) consume(ctx context.Context, id uint16, queue, tag string, d *delivery) {
	for {
		...
		case msg, ok := <-d.ch:
			if !ok {
				p.logger.Debug("Basic.Consume closed", "queue", queue)
			}
			p.logger.Debug("Basic.Deliver", "msg", msg)
...
func (p *Proxy) replyBasicGet(ctx context.Context, f *amqp091.MethodFrame, m *amqp091.BasicGet) error {
	id := f.Channel()
	uss := p.Upstreams()
	p.logger.Debug("Basic.Get", "queue", m.Queue)
	var got bool
	for i, us := range uss {
		ch, err := us.GetChannel(id)
		if err != nil {
		...
		p.logger.Debug("Basic.Get", "msg", msg, "client.DeliveryTag", dTag, "upstream.DeliveryTag", msg.DeliveryTag)
...
func (p *Proxy) replyBasicAck(_ context.Context, f *amqp091.MethodFrame, m *amqp091.BasicAck) error {
	...
	p.logger.Debug("Basic.Ack", "client.DeliveryTag", m.DeliveryTag, "upstream.DeliveryTag", tag, "multiple", m.Multiple)
...
func (p *Proxy) replyBasicNack(_ context.Context, f *amqp091.MethodFrame, m *amqp091.BasicNack) error {
	...
	tag := p.UpstreamDeliveryTag(m.DeliveryTag)
	p.logger.Debug("Basic.Nack", "client.DeliveryTag", m.DeliveryTag, "upstream.DeliveryTag", tag, "multiple", m.Multiple, "requeue", m.Requeue)
...
func (p *Proxy) replyBasicCancel(_ context.Context, f *amqp091.MethodFrame, m *amqp091.BasicCancel) error {
	...
	p.logger.Debug("Basic.Cancel", "consumer_tag", m.ConsumerTag)
	for _, ch := range chs {
...
func (p *Proxy) replyQueueDelete(_ context.Context, f *amqp091.MethodFrame, m *amqp091.QueueDelete) error {
	...
	p.logger.Debug("Queue.Delete", "queue", m.Queue)
	for _, ch := range chs {
...
func (p *Proxy) replyQueueBind(_ context.Context, f *amqp091.MethodFrame, m *amqp091.QueueBind) error {
	...
	p.logger.Debug("Queue.Bind", "queue", m.Queue, "exchange", m.Exchange, "routing_key", m.RoutingKey)
	for _, ch := range chs {
		if err := ch.QueueBind(
			m.Queue,
			m.RoutingKey,
...
func (p *Proxy) replyQueueUnbind(_ context.Context, f *amqp091.MethodFrame, m *amqp091.QueueUnbind) error {
	id := f.Channel()
	uss := p.Upstreams()
	p.logger.Debug("Queue.Unbind", "queue", m.Queue, "exchange", m.Exchange, "routing_key", m.RoutingKey)
	for _, us := range uss {
		ch, err := us.GetChannel(id)
		if err != nil {
			return fmt.Errorf("failed to get channel: %w", err)
...
func (p *Proxy) replyBasicQos(_ context.Context, f *amqp091.MethodFrame, m *amqp091.BasicQos) error {
	...
	p.logger.Debug("Basic.Qos", "prefetch_count", m.PrefetchCount, "global", m.Global)
	for _, ch := range chs {
		if err := ch.Qos(
			int(m.PrefetchCount),
			int(m.PrefetchSize),
...
func (p *Proxy) replyExchangeDeclare(_ context.Context, f *amqp091.MethodFrame, m *amqp091.ExchangeDeclare) error {
	...
	p.logger.Debug("Exchange.Declare", "exchange", m.Exchange)
	for _, ch := range chs {
...
func (p *Proxy) replyQueuePurge(_ context.Context, f *amqp091.MethodFrame, m *amqp091.QueuePurge) error {
	...
	p.logger.Debug("Queue.Purge", "queue", m.Queue)
	for _, ch := range chs {
