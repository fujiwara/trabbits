...
func (p *Proxy) replyChannelOpen(ctx context.Context, f *amqp091.MethodFrame, m *amqp091.ChannelOpen) error {
	id := f.Channel()
	p.sendProbeLog("c->t Channel.Open", "channel", id, "message", m)
...
func (p *Proxy) replyChannelClose(_ context.Context, f *amqp091.MethodFrame, m *amqp091.ChannelClose) error {
	id := f.Channel()
	p.sendProbeLog("c->t Channel.Close", "channel", id, "message", m)
...
func (p *Proxy) replyQueueDeclare(_ context.Context, f *amqp091.MethodFrame, m *amqp091.QueueDeclare) error {
	...
	p.sendProbeLog("c->t Queue.Declare", "message", m)
	if m.Queue == "" {
		m.Queue = AutoGenerateQueueNamePrefix + generateID()
		p.sendProbeLog("c->t Queue.Declare auto-generated name", "name", m.Queue)
	}
	var messages, consumers int
	var queueName string
	for i, ch := range chs {
		us := p.Upstream(i)
		p.sendProbeLog("t->u Queue.Declare", "upstream", us.String(), "message", m)
		q, err := us.QueueDeclareWithTryPassive(ch, m)
		if err != nil {
			return fmt.Errorf("failed to declare queue on upstream %s: %w", us.String(), err)
		}
		p.sendProbeLog("t<-u Queue.Declare result", "upstream", us.String(), "result", q)
		messages += q.Messages
	...
	p.sendProbeLog("c<-t Queue.Declare completed", "final_name", queueName, "total_messages", messages, "total_consumers", consumers)
...
func (p *Proxy) replyBasicPublish(ctx context.Context, f *amqp091.MethodFrame, m *amqp091.BasicPublish) error {
	...
	p.sendProbeLog("c->t Basic.Publish", "message", m)
...
func (p *Proxy) replyBasicConsume(ctx context.Context, f *amqp091.MethodFrame, m *amqp091.BasicConsume) error {
	...
	p.sendProbeLog("c->t Basic.Consume", "message", m)
	deliveries := make([]*delivery, 0, len(uss))
	for i, us := range uss {
		p.sendProbeLog("t->u Basic.Consume", "upstream", us.String())
...
func (p *Proxy) consume(ctx context.Context, id uint16, queue, tag string, d *delivery) {
	for {
		...
		case msg, ok := <-d.ch:
			if !ok {
				p.sendProbeLog("t<-u Basic.Consume closed", "queue", queue)
				return
			}
			p.sendProbeLog("t<-u Basic.Deliver", "exchange", msg.Exchange, "routing_key", msg.RoutingKey, "delivery_tag", msg.DeliveryTag)
...
func (p *Proxy) replyBasicGet(ctx context.Context, f *amqp091.MethodFrame, m *amqp091.BasicGet) error {
	id := f.Channel()
	uss := p.Upstreams()
	p.sendProbeLog("c->t Basic.Get", "message", m)
	var got bool
	for i, us := range uss {
		p.sendProbeLog("t->u Basic.Get", "upstream", us.String())
		ch, err := us.GetChannel(id)
		if err != nil {
			return fmt.Errorf("failed to get channel on upstream %s: %w", us.String(), err)
		}
		...
		p.sendProbeLog("t<-u Basic.Get received", "queue", m.Queue, "client_delivery_tag", dTag, "upstream_delivery_tag", msg.DeliveryTag)
...
func (p *Proxy) replyBasicAck(_ context.Context, f *amqp091.MethodFrame, m *amqp091.BasicAck) error {
	...
	p.sendProbeLog("c->t Basic.Ack", "message", m, "upstream_delivery_tag", tag)
...
func (p *Proxy) replyBasicNack(_ context.Context, f *amqp091.MethodFrame, m *amqp091.BasicNack) error {
	...
	tag := p.UpstreamDeliveryTag(m.DeliveryTag)
	p.sendProbeLog("c->t Basic.Nack", "message", m, "upstream_delivery_tag", tag)
...
func (p *Proxy) replyBasicCancel(_ context.Context, f *amqp091.MethodFrame, m *amqp091.BasicCancel) error {
	...
	p.sendProbeLog("c->t Basic.Cancel", "message", m)
	for i, ch := range chs {
		us := p.Upstream(i)
		p.sendProbeLog("t->u Basic.Cancel", "upstream", us.String())
...
func (p *Proxy) replyQueueDelete(_ context.Context, f *amqp091.MethodFrame, m *amqp091.QueueDelete) error {
	...
	p.sendProbeLog("c->t Queue.Delete", "message", m)
	for i, ch := range chs {
		us := p.Upstream(i)
		p.sendProbeLog("t->u Queue.Delete", "upstream", us.String())
...
func (p *Proxy) replyQueueBind(_ context.Context, f *amqp091.MethodFrame, m *amqp091.QueueBind) error {
	...
	p.sendProbeLog("c->t Queue.Bind", "message", m)
	for i, ch := range chs {
		us := p.Upstream(i)
		p.sendProbeLog("t->u Queue.Bind", "upstream", us.String())
		if err := ch.QueueBind(
			m.Queue,
			m.RoutingKey,
			m.Exchange,
...
func (p *Proxy) replyQueueUnbind(_ context.Context, f *amqp091.MethodFrame, m *amqp091.QueueUnbind) error {
	id := f.Channel()
	uss := p.Upstreams()
	p.sendProbeLog("c->t Queue.Unbind", "message", m)
	for _, us := range uss {
		p.sendProbeLog("t->u Queue.Unbind", "upstream", us.String())
		ch, err := us.GetChannel(id)
		if err != nil {
			return fmt.Errorf("failed to get channel on upstream %s: %w", us.String(), err)
		}
...
func (p *Proxy) replyBasicQos(_ context.Context, f *amqp091.MethodFrame, m *amqp091.BasicQos) error {
	...
	p.sendProbeLog("c->t Basic.Qos", "message", m)
	for i, ch := range chs {
		us := p.Upstream(i)
		p.sendProbeLog("t->u Basic.Qos", "upstream", us.String())
		if err := ch.Qos(
			int(m.PrefetchCount),
			int(m.PrefetchSize),
			m.Global,
...
func (p *Proxy) replyExchangeDeclare(_ context.Context, f *amqp091.MethodFrame, m *amqp091.ExchangeDeclare) error {
	...
	p.sendProbeLog("c->t Exchange.Declare", "message", m)
	for i, ch := range chs {
		us := p.Upstream(i)
		p.sendProbeLog("t->u Exchange.Declare", "upstream", us.String())
...
func (p *Proxy) replyQueuePurge(_ context.Context, f *amqp091.MethodFrame, m *amqp091.QueuePurge) error {
	...
	p.sendProbeLog("c->t Queue.Purge", "message", m)
	for i, ch := range chs {
		us := p.Upstream(i)
		p.sendProbeLog("t->u Queue.Purge", "upstream", us.String())
