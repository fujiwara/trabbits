...
func (u *Upstream) probeLog(message string, attrs ...any) {
	...
	attrs = append([]any{"upstream", u.name}, attrs...)
...
func (u *Upstream) GetChannel(id uint16) (*rabbitmq.Channel, error) {
	...
	if ch, ok := u.channel[id]; ok {
		u.probeLog("t->u got channel", "id", id)
		return ch, nil
	}
...
func (u *Upstream) CloseChannel(id uint16) error {
	u.mu.Lock()
	defer u.mu.Unlock()
	u.probeLog("t->u closing channel", "id", id)
	if ch, ok := u.channel[id]; !ok {
		// channel not found
	} else {
...
func (u *Upstream) CleanupAutoGeneratedQueue(name string) {
	...
	fn := func(ch *rabbitmq.Channel) {
		u.probeLog("t->u auto-generated queue cleanup at shutdown", "queue", name)
		if n, err := ch.QueueDelete(name, false, false, false); err != nil {
			u.logger.Warn("failed to delete auto-generated queue", "queue", name, "error", err)
		} else {
			u.probeLog("t<-u auto-generated queue deleted", "queue", name, "purged_messages", n)
		}
	}
...
func (u *Upstream) RegisterEmulatedAutoDeleteQueue(queueName string) {
	...
	fn := func(ch *rabbitmq.Channel) {
		u.probeLog("t->u deleting queue with emulated auto_delete", "queue", queueName)
		if _, err := ch.QueueDelete(queueName, false, false, false); err != nil {
			...
			if amqpErr, ok := err.(*rabbitmq.Error); ok && amqpErr.Code == amqp091.NotFound {
				u.probeLog("t<-u queue already deleted", "queue", queueName)
				return
			}
...
func (u *Upstream) QueueDeclareWithTryPassive(ch *rabbitmq.Channel, m *amqp091.QueueDeclare) (rabbitmq.Queue, error) {
	...
	if u.queueOpts != nil && u.queueOpts.TryPassive {
		...
		u.probeLog("t->u trying passive declare", "queue", m.Queue)
		q, err := ch.QueueDeclare(m.Queue, false, false, false, true, nil)
		if err == nil {
			u.probeLog("t<-u passive declare succeeded", "queue", m.Queue)
			queueExisted = true
