...
// probeLog sends a probe log if the function is set
func (u *Upstream) probeLog(message string, attrs ...any) {
	if u.probeLogFunc != nil {
		u.probeLogFunc(message, attrs...)
	}
}
...
func (u *Upstream) NewChannel(id uint16) (*rabbitmq.Channel, error) {
	...
	u.channel[id] = ch
	u.probeLog("t->u channel created", "id", id, "upstream", u.name)
...
func (u *Upstream) GetChannel(id uint16) (*rabbitmq.Channel, error) {
	...
	if ch, ok := u.channel[id]; ok {
		u.probeLog("t->u got channel", "id", id, "upstream", u.name)
		return ch, nil
	}
...
func (u *Upstream) CloseChannel(id uint16) error {
	u.mu.Lock()
	defer u.mu.Unlock()
	u.probeLog("t->u closing channel", "id", id, "upstream", u.name)
	if ch, ok := u.channel[id]; !ok {
		// channel not found
	} else {
...
func (u *Upstream) CleanupAutoGeneratedQueue(name string) {
	...
	fn := func(ch *rabbitmq.Channel) {
		u.probeLog("t->u auto-generated queue cleanup at shutdown", "queue", name, "upstream", u.name)
...
func (u *Upstream) RegisterEmulatedAutoDeleteQueue(queueName string) {
	...
	fn := func(ch *rabbitmq.Channel) {
		u.probeLog("t->u deleting queue with emulated auto_delete", "queue", queueName, "upstream", u.name)
		if _, err := ch.QueueDelete(queueName, false, false, false); err != nil {
			...
			if amqpErr, ok := err.(*rabbitmq.Error); ok && amqpErr.Code == amqp091.NotFound {
				u.probeLog("t<-u queue already deleted", "queue", queueName, "upstream", u.name)
				return
			}
...
func (u *Upstream) QueueDeclareArgs(m *amqp091.QueueDeclare) (name string, durable bool, autoDelete bool, exclusive bool, noWait bool, args rabbitmq.Table) {
	...
	if attr := u.queueAttr; attr == nil {
		...
		u.probeLog("t->u overriding queue attributes", "queue", m.Queue, "attributes", *attr, "upstream", u.name)
...
func (u *Upstream) QueueDeclareWithTryPassive(ch *rabbitmq.Channel, m *amqp091.QueueDeclare) (rabbitmq.Queue, error) {
	...
	if u.queueOpts != nil && u.queueOpts.TryPassive {
		...
		u.probeLog("t->u trying passive declare", "queue", m.Queue, "upstream", u.name)
		q, err := ch.QueueDeclare(m.Queue, false, false, false, true, nil)
		if err == nil {
			u.probeLog("t<-u passive declare succeeded", "queue", m.Queue, "upstream", u.name)
			queueExisted = true
		...
		if amqpErr, ok := err.(*rabbitmq.Error); ok && amqpErr.Code == amqp091.NotFound {
			u.probeLog("t<-u queue not found, falling back to normal declare", "queue", m.Queue, "upstream", u.name)
	...
	if !queueExisted && u.shouldEmulateAutoDelete(m) {
		u.probeLog("t->u registering queue for emulated auto_delete", "queue", q.Name, "upstream", u.name)
