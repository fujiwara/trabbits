...
run-bench-trabbits: trabbits
	./trabbits run --config <(echo '{"upstreams":[{"host":"127.0.0.1","port":5672}]}') --port 5673

run-bench-servers: run-bench-upstream run-bench-trabbits

run-bench-client-direct:
	docker run -it --rm --network=host pivotalrabbitmq/perf-test:latest --uri amqp://admin:admin@127.0.0.1:5672 -z 30

run-bench-client-proxy:
	docker run -it --rm --network=host pivotalrabbitmq/perf-test:latest --uri amqp://admin:admin@127.0.0.1:5673 -z 30
